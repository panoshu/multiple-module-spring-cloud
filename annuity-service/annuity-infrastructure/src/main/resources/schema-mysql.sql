...
CREATE TABLE IF NOT EXISTS `t_annuity_application` (
    ...
    `business_type`           VARCHAR(64)   DEFAULT NULL                      COMMENT '业务类型: ACC_PLAN_CREATE/MODIFY/DELETE',
    ...
    `channel`                 VARCHAR(64)   DEFAULT NULL                      COMMENT '年金渠道',
    ...
    `parsed_json_file_id`     VARCHAR(64)   DEFAULT NULL                      COMMENT '解析后 JSON 文件 ID',
    ...
    `current_step`            VARCHAR(64)   DEFAULT NULL                      COMMENT '当前流程步骤',
    ...
    PRIMARY KEY (`id`),
    KEY `idx_annuity_application_batch_id`        (`batch_id`),
    KEY `idx_annuity_application_form_id`         (`form_id`),
    KEY `idx_annuity_application_parsed_file_id`  (`parsed_json_file_id`),
    KEY `idx_annuity_application_status`          (`status`),
    KEY `idx_annuity_application_current_step`    (`current_step`),
    KEY `idx_annuity_application_customer_no`     (`customer_no`),
    KEY `idx_annuity_application_create_time`     (`create_time`)
...
CREATE TABLE IF NOT EXISTS `t_annuity_batch` (
    ...
    `status`                  VARCHAR(32)   DEFAULT NULL                      COMMENT '批次状态: CREATED/PROCESSING/PARTIAL_FAILED/FAILED/COMPLETED',
    ...
    PRIMARY KEY (`id`),
    KEY `idx_annuity_batch_status`         (`status`),
    KEY `idx_annuity_batch_customer_no`    (`customer_no`),
    KEY `idx_annuity_batch_business_type`  (`business_type`),
    KEY `idx_annuity_batch_create_time`    (`create_time`)
...
CREATE TABLE IF NOT EXISTS `t_annuity_form` (
    ...
    PRIMARY KEY (`id`),
    KEY `idx_annuity_form_batch_id`      (`batch_id`),
    KEY `idx_annuity_form_status`        (`form_status`),
