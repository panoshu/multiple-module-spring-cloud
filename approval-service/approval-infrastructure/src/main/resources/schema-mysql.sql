...
(
  64
) NOT NULL COMMENT '审批流ID',
  `flow_name` VARCHAR
(
  255
) NOT NULL COMMENT '审批流名称',
...
(
  20
) NOT NULL COMMENT '状态：ACTIVE-有效，DEPRECATED-已废弃',
  `created_by` VARCHAR
(
  64
...
(
  20
) NOT NULL COMMENT '节点类型：SPECIFIED_PLAN-指定方案，SAME_PLAN-同方案，LEVEL_UP-上一级',
  `specified_plan_id` VARCHAR
(
  64
) DEFAULT NULL COMMENT '指定方案ID',
  `terminal_level` INT DEFAULT NULL COMMENT '终止级别',
  `approver_type` VARCHAR
(
  20
) NOT NULL COMMENT '审批人类型：SPECIFIED_USER-指定用户，SPECIFIED_ROLE-指定角色',
  `approver_ids` TEXT DEFAULT NULL COMMENT '审批人ID列表（JSON）',
  `role_ids` TEXT DEFAULT NULL COMMENT '角色ID列表（JSON）',
  `sign_mode` VARCHAR
(
  20
) NOT NULL COMMENT '签批模式：OR_SIGN-或签，AND_SIGN-会签',
  `created_by` VARCHAR
(
  64
) NOT NULL COMMENT '创建人',
...
(
  128
) NOT NULL COMMENT '业务单号',
  `current_node_order` INT NOT NULL COMMENT '当前节点顺序',
  `status` VARCHAR
(
  20
) NOT NULL COMMENT '实例状态：PENDING-待审批，APPROVING-审批中，APPROVED-已通过，REJECTED-已拒绝，WITHDRAWN-已撤回',
  `initiator_plan` VARCHAR
(
  64
) DEFAULT NULL COMMENT '发起人方案',
  `current_plan` VARCHAR
(
  64
) DEFAULT NULL COMMENT '当前方案',
  `created_by` VARCHAR
(
  64
) NOT NULL COMMENT '创建人',
...
(
  ...
  KEY `idx_business_application_id`
(
...
(
  20
) NOT NULL COMMENT '执行状态：PENDING-待执行，APPROVED-已通过，REJECTED-已拒绝，SKIPPED-已跳过',
...
(
  64
) NOT NULL COMMENT '审批记录ID',
  `execution_id` VARCHAR
(
  64
) NOT NULL COMMENT '节点执行ID',
  `approver_id` VARCHAR
(
  64
) NOT NULL COMMENT '审批人ID',
  `action` VARCHAR
(
  20
) NOT NULL COMMENT '审批动作：APPROVE-通过，REJECT-拒绝，TRANSFER-转交',
  `opinion` VARCHAR
(
  500
