...
COMMENT ON TABLE t_approval_flow IS '审批流配置表';
COMMENT ON COLUMN t_approval_flow.id IS '审批流ID';
COMMENT ON COLUMN t_approval_flow.flow_name IS '审批流名称';
COMMENT ON COLUMN t_approval_flow.business_type IS '业务类型';
COMMENT ON COLUMN t_approval_flow.match_rules IS '匹配规则（JSON）';
COMMENT ON COLUMN t_approval_flow.flow_version IS '版本号';
COMMENT ON COLUMN t_approval_flow.status IS '状态：ACTIVE-有效，DEPRECATED-已废弃';
COMMENT ON COLUMN t_approval_flow.created_by IS '创建人';
COMMENT ON COLUMN t_approval_flow.updated_by IS '更新人';
COMMENT ON COLUMN t_approval_flow.create_time IS '创建时间';
COMMENT ON COLUMN t_approval_flow.update_time IS '更新时间';
COMMENT ON COLUMN t_approval_flow.deleted IS '删除标记';
COMMENT ON COLUMN t_approval_flow.version IS '乐观锁版本号';
...
CREATE TABLE IF NOT EXISTS t_approval_node (
  ...
  specified_plan_id VARCHAR(64),
  terminal_level INT,
  approver_type VARCHAR(20) NOT NULL,
  approver_ids TEXT,
  role_ids TEXT,
  sign_mode VARCHAR(20) NOT NULL,
  created_by VARCHAR(64) NOT NULL,
  updated_by VARCHAR(64),
  create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  update_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
COMMENT ON TABLE t_approval_node IS '审批节点配置表';
...
CREATE TABLE IF NOT EXISTS t_approval_instance (
  ...
  flow_id VARCHAR(64) NOT NULL,
  flow_version INT NOT NULL,
  business_application_id VARCHAR(64) NOT NULL,
  business_type VARCHAR(64) NOT NULL,
  business_no VARCHAR(128) NOT NULL,
  current_node_order INT NOT NULL,
  status VARCHAR(20) NOT NULL,
  initiator_plan VARCHAR(64),
...
COMMENT ON TABLE t_approval_instance IS '审批实例表';
...
COMMENT ON TABLE t_approval_node_execution IS '节点执行记录表';
...
COMMENT ON TABLE t_approval_record IS '审批记录表';