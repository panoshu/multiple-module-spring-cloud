 ...
 * </ol>
 *
 * <p>每个启用渠道独立持有一个 {@link StpLogic} 实例,通过 {@link SaTokenConfig#setTokenName}
 * 设置渠道专属 Token Header 名(satoken-internet / satoken-hq / satoken-branch),
 * 实现 Token 互不干扰,便于独立管理。
 *
 * <p>启用渠道由 {@link GatewayChannelProperties#enabled()} 配置驱动，
 * 各网关按需注册，而非固定枚举全量注册。
 *
...
public class ChannelAwareSaRouter {
  ...
  private final List<ChannelType> enabledChannels;

  public ChannelAwareSaRouter(GatewayChannelProperties channelProperties) {
    this.enabledChannels = channelProperties.enabled();
    ...
    log.info("ChannelAwareSaRouter 初始化完成: channels={}",
      enabledChannels.stream().map(ChannelType::loginType).toList());
  }
   ...
   * <ul>
   *   <li>路径匹配启用渠道前缀(如 /internet/**):调用对应 StpLogic.checkLogin,
   *       未登录抛 NotLoginException 由 setError 处理</li>
   *   <li>路径不匹配任何启用渠道前缀(如 /actuator/**, /favicon.ico):
   *       返回 null,表示公共接口,跳过登录校验</li>
   * </ul>
   *
   * @return 命中的渠道类型（仅限已启用渠道）;公共接口返回 null
   */
  public ChannelType matchAndCheckLogin() {
    SaRequest request = SaHolder.getRequest();
    String path = request.getRequestPath();
    ChannelType channel = fromPath(path);
    if (channel == null) {
      return null;
    }
    StpLogic stpLogic = stpLogicMap.get(channel);
    if (stpLogic == null) {
      // 路径命中某渠道前缀但该渠道未在网关启用（如 internet 网关收到 /hq 请求）
      return null;
    }
    stpLogic.checkLogin();
    return channel;
  ...
  public ChannelType getChannelByLoginType(String loginType) {
    return ChannelType.fromName(loginType);
  }
   ...
   * 获取当前网关启用的渠道列表。
   *
   * <p>供 SessionContextInjector 遍历启用渠道（而非枚举全量），
   * 避免在单一渠道网关中误识别其他渠道。
   *
   * @return 启用的渠道列表
   */
  public List<ChannelType> enabledChannels() {
    return enabledChannels;
  }

  private ChannelType fromPath(String path) {
    ChannelType channel = ChannelType.fromPath(path);
    if (channel == null || !enabledChannels.contains(channel)) {
      return null;
    }
    return channel;
  }
  ...
  /**
   * 配置默认 StpLogic 识别本网关所有启用渠道 token.
   *
   * <p>管理类 API（非渠道前缀路径）使用默认 StpLogic 校验登录态，
   * 默认 StpLogic 读取本网关启用渠道的 Header，用户携带任一启用渠道 token 都能通过校验。
   *
   * <p>sa-token 1.45.0 的 {@link SaTokenConfig#setTokenName(String)} 仅支持单个 token 名称，
   * 因此通过覆写 {@link StpLogic#getTokenValueNotCut()} 遍历所有启用渠道 Header 实现多 Header 识别。
   *
  ...
  public void configureDefaultStpLogic() {
    ...
    log.info("默认 StpLogic 配置完成: 识别本网关启用渠道 token: {}", channelHeaders);
  }
}