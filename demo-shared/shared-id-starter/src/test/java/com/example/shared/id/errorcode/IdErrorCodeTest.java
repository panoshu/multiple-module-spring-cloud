 ...
 * <ul>
 *   <li>5 位纯数字</li>
 *   <li>码段 14001-14099（shared-id-starter）</li>
...
class IdErrorCodeTest {
  ...
  @ParameterizedTest(name = "{0} 的 code 应在 14001-14099 码段")
  @EnumSource(IdErrorCode.class)
  @DisplayName("所有 code 应落在 shared-id-starter 码段 14001-14099")
  void codeShouldBeInIdSegment(IdErrorCode error) {
    int code = Integer.parseInt(error.code());
    assertThat(code)
        .as("%s 的 code 应在 14001-14099 区间", error.name())
        .isBetween(14001, 14099);
  }
  ...
  @Test
  @DisplayName("ID_GEN_ERROR 码值应为 14001")
  void idGenErrorShouldHaveCorrectCode() {
    assertThat(IdErrorCode.ID_GEN_ERROR.code()).isEqualTo("14001");
    assertThat(IdErrorCode.ID_GEN_ERROR.message()).isEqualTo("ID生成异常");
  }
}
