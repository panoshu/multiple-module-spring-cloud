...
class LockErrorDefinitionTest {
  ...
  void codeShouldBeUnique() {
    long distinctCount = Arrays.stream(LockErrorDefinition.values())
      .map(LockErrorDefinition::getCode)
      .distinct()
      .count();
  ...
  void getLockFailedShouldExist() {
    assertThat(LockErrorDefinition.GET_LOCK_FAILED.getCode()).isEqualTo("SHARED.LOCK.0001");
    assertThat(LockErrorDefinition.GET_LOCK_FAILED.getMessage()).isEqualTo("获取锁失败");
  }
}
