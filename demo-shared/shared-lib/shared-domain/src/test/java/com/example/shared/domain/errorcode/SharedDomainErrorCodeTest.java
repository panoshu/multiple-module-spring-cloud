...
class SharedDomainErrorCodeTest {
  ...
  void message_should_return_defined_text() {
    assertThat(SharedDomainErrorCode.ENTITY_NOT_FOUND.getMessage()).isNotBlank();
    assertThat(SharedDomainErrorCode.INVALID_DATA.getMessage()).isNotBlank();
    assertThat(SharedDomainErrorCode.INVALID_OPERATION.getMessage()).isNotBlank();
  }
  ...
  void code_should_be_unique() {
    long distinctCount = java.util.Arrays.stream(SharedDomainErrorCode.values())
      .map(SharedDomainErrorCode::getCode)
      .distinct()
      .count();
