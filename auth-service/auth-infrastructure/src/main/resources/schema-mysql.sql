...
(
  ...
  active_teller_key         VARCHAR(32) GENERATED ALWAYS AS (CASE
                                                               WHEN deleted = 0 AND status IN ('PENDING', 'AUTHORIZED')
                                                                 THEN teller_account_id
                                                               ELSE NULL END) STORED COMMENT '活跃柜员唯一键(生成列)',
  PRIMARY KEY (id),
...
(
  32
) NOT NULL COMMENT '用户ID(ULID)',
  user_type VARCHAR
(
  32
) NOT NULL COMMENT '用户类型: AGENT/OPERATOR/TELLER',
  identity_type VARCHAR
(
  32
) NOT NULL COMMENT '证件类型: ID_CARD/PASSPORT 等',
  identity_number VARCHAR
(
  64
) NOT NULL COMMENT '证件号码',
  mobile VARCHAR
(
  32
) COMMENT '手机号',
...
(
  16
) NOT NULL COMMENT '用户状态: ACTIVE/FROZEN/DISABLED',
  created_by VARCHAR
(
  64
) NOT NULL,
...
(
  ...
  UNIQUE KEY uk_t_auth_user_identity
(
  identity_type,
  identity_number,
  deleted
),
  KEY idx_t_auth_user_mobile
(
  mobile,
  deleted
),
  KEY idx_t_auth_user_status
(
  status,
  deleted
)
...
(
  32
) NOT NULL COMMENT '凭证ID(ULID)',
  credential_type VARCHAR
(
  32
) NOT NULL COMMENT '凭证类型: PASSWORD/U_KEY',
  owner_type VARCHAR
(
  32
) NOT NULL COMMENT '持有者类型: UserCredentialOwner/CustomerCredentialOwner/PlanCredentialOwner',
  owner_id VARCHAR
(
  64
) NOT NULL COMMENT '持有者ID',
...
(
  16
) NOT NULL COMMENT '状态: ACTIVE/REVOKED/DISABLED',
  -- PasswordCredential 专属字段
  user_no VARCHAR
(
  32
) COMMENT '密码所属账号(Password 专属)',
  password_hash VARCHAR
(
  255
) COMMENT '密码哈希(Password 专属)',
  -- UKeyCredential 专属字段
  key_serial VARCHAR
(
  128
) COMMENT 'UKey 序列号(UKey 专属)',
  -- 通用字段
  created_by VARCHAR
(
  64
...
(
  ...
  UNIQUE KEY uk_t_auth_credential_ukey_serial
(
  key_serial,
  deleted
),
  KEY idx_t_auth_credential_owner
(
  owner_type,
  owner_id,
  deleted
),
  KEY idx_t_auth_credential_type
(
  credential_type,
  status,
  deleted
...
(
  32
) NOT NULL COMMENT '范围维度: GLOBAL/CUSTOMER/PRODUCT/PLAN',
  scope_value VARCHAR
(
  64
) COMMENT '范围值，GLOBAL 时为 NULL',
  permissions JSON NOT NULL COMMENT '权限集合 JSON 数组',
  status VARCHAR
(
  16
) NOT NULL COMMENT '状态: DRAFT/EFFECTIVE/INACTIVE',
  created_by VARCHAR
(
  64
) NOT NULL,
...
(
  ...
  KEY idx_t_auth_role_template_code
(
  role_code,
  status,
  deleted
),
  KEY idx_t_auth_role_template_scope
(
  scope_dimension,
  scope_value,
  deleted
...
(
  32
) NOT NULL COMMENT '维度: PLAN/CUSTOMER',
  scope_value VARCHAR
(
  64
) NOT NULL COMMENT '范围值',
  mode VARCHAR
(
  16
) NOT NULL COMMENT '可见性模式: SHOW_ALL/EXCLUSIVE_ONLY',
  created_by VARCHAR
(
  64
) NOT NULL,
...
(
  ...
  UNIQUE KEY uk_t_auth_role_visibility_scope
(
  dimension,
  scope_value,
  deleted
...
(
  32
) NOT NULL COMMENT '范围维度: PLAN/CUSTOMER/PRODUCT/GLOBAL',
  scope_value VARCHAR
(
  64
) NOT NULL COMMENT '范围值',
...
(
  1
) NOT NULL DEFAULT 0 COMMENT '是否级联下级客户（仅 CUSTOMER 维度有效）',
  status VARCHAR
(
  16
) NOT NULL COMMENT '状态: ACTIVE/DEACTIVATED',
  created_by VARCHAR
(
  64
) NOT NULL,
...
(
  ...
  KEY idx_t_auth_assignment_user
(
  user_no,
  status,
  deleted
),
  KEY idx_t_auth_assignment_scope
(
  scope_dimension,
  scope_value,
  deleted
),
  KEY idx_t_auth_assignment_role
(
  role_code,
  status,
  deleted
...
(
  32
) NOT NULL COMMENT 'Grant ID(ULID)',
...
(
  32
) NOT NULL COMMENT '类型: BASE/DELEGATE_WHOLESALE/DELEGATE_SELECTIVE',
  origin VARCHAR
(
  32
) NOT NULL COMMENT '来源: HQ_CONFIG/PLAN_DELEGATE/CUSTOMER_TO_AGENT/ROLE_TEMPLATE',
  effect VARCHAR
(
  16
) NOT NULL COMMENT '效果: ALLOW/DENY',
...
(
  16
) NOT NULL COMMENT '状态: DRAFT/PENDING_APPROVAL/EFFECTIVE/REJECTED/REVOKED',
...
(
  ...
  KEY idx_t_auth_grant_status
(
  status,
  deleted
),
  KEY idx_t_auth_grant_type
(
  grant_type,
  status,
  deleted
),
  KEY idx_t_auth_grant_origin
(
  origin,
  status,
  deleted
),
  KEY idx_t_auth_grant_validity
(
  validity_start,
  validity_end,
  deleted
),
  KEY idx_t_auth_grant_source_plan
(
  source_plan_no,
  deleted
),
  KEY idx_t_auth_grant_target_plan
(
  target_plan_no,
  deleted
)
...
(
  64
) COMMENT '操作编码（NULL=整个业务）',
  category VARCHAR
(
  16
) NOT NULL COMMENT '权限类别: BUSINESS/PLATFORM',
  source VARCHAR
(
  16
) NOT NULL COMMENT '来源: API/MANUAL',
  controller VARCHAR
(
  255
) COMMENT '控制器类名',
...
(
  ...
  auto_registered TINYINT
(
  1
) NOT NULL DEFAULT 1 COMMENT '是否自动注册（被扫描器标记为 stale 时置 0）',
  created_by VARCHAR
(
  64
) NOT NULL,
...
(
  ...
  UNIQUE KEY uk_permission_item_biz_action
(
  business_code,
  action_code
),
  KEY idx_permission_item_category
(
  category
),
  KEY idx_permission_item_group
(
  category_group
)
...
(
  32
) NOT NULL COMMENT '开通记录ID(ULID)',
  customer_no VARCHAR
(
  32
) NOT NULL COMMENT '客户编号',
...
(
  ...
  UNIQUE KEY uk_t_auth_customer_channel_entitlement_customer
(
  customer_no,
  deleted
)
