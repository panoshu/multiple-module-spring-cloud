...
(
  64
) NOT NULL COMMENT '文件ID（FileId）',
  `original_name` VARCHAR
(
  512
) DEFAULT NULL COMMENT '原始文件名（PENDING_UPLOAD 阶段允许 NULL）',
  `size` BIGINT DEFAULT NULL COMMENT '文件大小（字节，PENDING_UPLOAD 阶段允许 NULL）',
  `content_type` VARCHAR
(
  128
) DEFAULT NULL COMMENT 'MIME 类型',
  `md5` VARCHAR
(
  64
) DEFAULT NULL COMMENT '内容 MD5 指纹（历史字段，新数据使用 digest）',
  `digest` VARCHAR
(
  128
) DEFAULT NULL COMMENT '内容摘要（SM3）',
  `digest_algorithm` VARCHAR
(
  20
) DEFAULT 'SM3' COMMENT '摘要算法: SM3',

  `target_id` VARCHAR
(
  64
) NOT NULL COMMENT '存储目标 ID',
  `storage_type` VARCHAR
(
  20
) NOT NULL COMMENT '存储类型: LOCAL/OSS/NAS',
  `storage_key` VARCHAR
(
  1024
) DEFAULT NULL COMMENT '后端内部 key/path（PENDING_UPLOAD 阶段允许 NULL）',

  `usage` VARCHAR
(
  20
) NOT NULL COMMENT '文件用途: SOURCE/PARSED/EXPORT/ARCHIVE',
  `biz_type` VARCHAR
(
  64
...
(
  64
) DEFAULT NULL COMMENT '来源系统标识',
  `business_batch_id` VARCHAR
(
  64
) DEFAULT NULL COMMENT '业务批次号',
...
(
  20
) NOT NULL COMMENT '文件状态: PENDING_UPLOAD/UPLOADED/DELETED',
  `uploaded_by` VARCHAR
(
  64
) DEFAULT NULL COMMENT '上传人',
...
(
  ...
  KEY `idx_file_metadata_batch_id`
(
  `business_batch_id`
),
  KEY `idx_file_metadata_usage_biz_type`
(
  `usage`,
  `biz_type`
),
  KEY `idx_file_metadata_status_expires`
(
  `status`,
  `expires_at`
),
  KEY `idx_file_metadata_target_id`
(
  `target_id`
)
...
(
  64
) NOT NULL COMMENT '流水ID（FileAccessLogId）',
  `file_id` VARCHAR
(
  64
) NOT NULL COMMENT '文件ID（FileId）',
...
(
  20
) NOT NULL COMMENT '动作类型: APPLY=申请token, ACCESS=实际访问',
  `usage` VARCHAR
(
  20
) NOT NULL COMMENT '文件用途: SOURCE/PARSED/EXPORT/ARCHIVE',
...
(
  64
) NOT NULL COMMENT '客户号',
  `product_no` VARCHAR
(
  64
) NOT NULL COMMENT '产品号',
  `operator` VARCHAR
(
  64
) NOT NULL COMMENT '操作人（UserNo）',
  `source_app` VARCHAR
(
  64
) DEFAULT NULL COMMENT '来源系统标识',
...
(
  64
) DEFAULT NULL COMMENT '来源 IP（仅 ACCESS 记录）',
  `token_hash` VARCHAR
(
  128
) NOT NULL COMMENT 'token SHA-256 哈希，用于关联 APPLY 与 ACCESS 记录',
  `result` VARCHAR
(
  20
) NOT NULL COMMENT '访问结果: SUCCESS/FAIL/EXPIRED/REJECTED',
  `fail_reason` VARCHAR
(
  512
) DEFAULT NULL COMMENT '失败原因（result != SUCCESS 时填充）',
...
(
  ...
  KEY `idx_access_log_file_id`
(
  `file_id`
),
  KEY `idx_access_log_token_hash`
(
  `token_hash`
),
  KEY `idx_access_log_action_time`
(
  `action`,
  `occur_at`
),
  KEY `idx_access_log_customer_product`
(
  `customer_no`,
  `product_no`,
  `occur_at`
...
(
  64
) NOT NULL COMMENT '任务ID（FileTaskId）',
  `biz_type` VARCHAR
(
  64
...
(
  64
) DEFAULT NULL COMMENT '模板编码',
  `source_file_name` VARCHAR
(
  512
) DEFAULT NULL COMMENT '源文件名',
  `source_file_id` VARCHAR
(
  64
) DEFAULT NULL COMMENT '源文件ID（FileId）',
  `status` VARCHAR
(
  32
) NOT NULL COMMENT '任务状态: PENDING/PROCESSING/SUCCESS/FAILED/CANCELLED',
  `error_policy` VARCHAR
(
  32
) DEFAULT NULL COMMENT '错误处理策略: SKIP/ABORT/QUARANTINE',
  `split_keys` VARCHAR
(
  255
) DEFAULT NULL COMMENT '拆分键列表（逗号分隔）',
...
(
  ...
  KEY `idx_file_parse_task_biz_status`
(
  `biz_type`,
  `status`
),
  KEY `idx_file_parse_task_source_file`
(
  `source_file_id`
),
  KEY `idx_file_parse_task_create_time`
(
  `create_time`
)
...
(
  64
) NOT NULL COMMENT '子任务ID（SubTaskId）',
  `file_task_id` VARCHAR
(
  64
) NOT NULL COMMENT '关联解析任务ID（FileTaskId）',
  `biz_type` VARCHAR
(
  64
) NOT NULL COMMENT '业务类型',
...
(
  32
) NOT NULL COMMENT '子任务状态: PENDING/PROCESSING/SUCCESS/FAILED',
...
(
  ...
  KEY `idx_file_sub_task_data_task_id`
(
  `file_task_id`
),
  KEY `idx_file_sub_task_data_split_key`
(
  `file_task_id`,
  `split_key_value`
),
  KEY `idx_file_sub_task_data_status`
(
  `status`
),
  KEY `idx_file_sub_task_data_expires_at`
(
  `expires_at`
)
...
(
  64
) NOT NULL COMMENT '模板配置ID（TemplateConfigId）',
  `biz_type` VARCHAR
(
  64
) NOT NULL COMMENT '业务类型',
...
(
  64
) DEFAULT NULL COMMENT '目标模板引用',
  `target_mapping` JSON DEFAULT NULL COMMENT '目标映射（JSON）',
  `status` VARCHAR
(
  32
) NOT NULL COMMENT '状态: DRAFT/ACTIVE/DEPRECATED',
...
(
  ...
  UNIQUE KEY `uk_file_template_config_biz_version`
(
  `biz_type`,
  `template_version`
),
  KEY `idx_file_template_config_biz_status`
(
  `biz_type`,
  `status`
),
  KEY `idx_file_template_config_effective`
(
  `biz_type`,
  `effective_from`,
  `effective_to`
